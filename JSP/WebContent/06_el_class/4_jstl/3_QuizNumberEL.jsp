...
<!-- 3. 세션의 값을 가져와 answer 변수에 지정 / scope으로 지정->공유가능 -->
...
<c:choose>
	<c:when test='${param.guess gt answer}'>작음</c:when>
	<c:when test='${param.guess lt answer}'>큼</c:when>
	<c:otherwise>통과</c:otherwise>
</c:choose>
</c:if>
